...
SELECT DISTINCT ?t WHERE {
  GRAPH <https://integratedsemantics.org/flexible-graphrag/kg> { ?s rdf:type ?t }
} ORDER BY ?t"""
...
class FusekiLangChainAdapter(RDFStoreAdapter):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Pass credentials separately — _LazyRdfGraph uses requests with
        # HTTPBasicAuth rather than embedding them in the URL (urllib does not
        # support user:pass@ and misreads it as the hostname).
        self.lc_graph = _LazyRdfGraph(clean_endpoint, username=username, password=password)
