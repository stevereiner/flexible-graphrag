...
class GraphDBLangChainAdapter(RDFStoreAdapter):
        ...
        try:
            ...
            if "Missing graph" in str(e) and not ontology_file:
                ...
                _stub = (
                    "@prefix owl: <http://www.w3.org/2002/07/owl#> .\n"
                    "@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> .\n"
                    "<https://integratedsemantics.org/flexible-graphrag/ontology#Entity> "
                    "a owl:Class ; rdfs:label \"Entity\" .\n"
                )
    ...
    def _patch_schema_with_instance_ns(self) -> None:
        ...
        preamble = (
            ...
            f"#    ?x rdfs:label ?name  FILTER(CONTAINS(LCASE(?name), 'acme'))\n"
            f"# 3. Use ONLY predicates listed below (actually present in the store).\n"
            f"#    The ontology may define more properties than are actually used.\n"
            f"#\n"
            f"# === PREDICATES ACTUALLY IN THE STORE ===\n"
            f"{pred_examples}\n"
            f"#\n"
            f"# === EXAMPLE QUERY PATTERN ===\n"
            f"# Question: Who works for Acme?\n"
