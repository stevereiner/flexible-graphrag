...
class SynonymFusion:
    ...
    def from_config(cls, config: "AppSettings", llm) -> "SynonymFusion":
        ...
        """
        scope_raw = (
            getattr(config, "synonym_exploder_scope", "") or "langchain_pg_vector"
        ).strip().lower()
        ...
        try:
            from langchain.graph.retrievers.synonym_rewriter import SynonymExpander
    ...
    @property
    def tags(self) -> set:
        """Active scope tags (empty set when scope=all or disabled)."""
    ...
    def wrap(self, retriever: BaseRetriever, tag: str) -> BaseRetriever:
        ...
        if tag in self._active_tags:
            from langchain.graph.retrievers.li_synonym_retriever import SynonymExpanderRetriever

            return SynonymExpanderRetriever(base=retriever, expander=self._expander)
        return retriever

    def restrict_tags(self, active_tags: set) -> None:
        """Remove any scope tags that have no active retriever.

        Called after all retrievers are built so that the expander only
        holds tags that will actually be exercised.  Unused tags would
        never trigger a synonym expansion but could confuse diagnostics.
        """
        removed = self._active_tags - active_tags
        if removed:
            self._active_tags = self._active_tags & active_tags
            logger.debug(
                "SynonymFusion: pruned inactive scope tags %s "
                "(no retriever matched; update SYNONYM_EXPLODER_SCOPE in .env to silence this)",
                sorted(removed),
            )
